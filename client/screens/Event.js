...
export default class Event extends React.Component {
  ...
  getFeatured = () => {
    if (this.state.eventData) {
      const featuredData = this.state.eventData.filter(
  ...
  getHot = () => {
    if (this.state.eventData) {
      const hotData = this.state.eventData.filter(
        (items) => items.hot === true
      );
      this.setState({
        hotData: hotData,
      });
      ...
      <>
        ...
        <ScrollView style={styles.home}>
          <View style={styles.featuredContainer}>
            ...
            <ScrollView horizontal={true} style={styles.featuredScroll}>
              {this.state.featuredData.length > 0 ? (
                this.state.featuredData.map((items) => {
                  return (
                    <FeaturedCard
                      key={items.long}
                      featuredData={items}
